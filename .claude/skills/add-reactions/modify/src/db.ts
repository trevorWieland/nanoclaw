...
export function getReactionsByUser(reactorJid: string, limit: number = 50): Reaction[] {
  return db
    .prepare(`SELECT * FROM reactions WHERE reactor_jid = ? ORDER BY timestamp DESC LIMIT ?`)
...
export function getReactionStats(chatJid?: string): Array<{
  ...
  return chatJid ? (db.prepare(sql).all(chatJid) as Result[]) : (db.prepare(sql).all() as Result[]);
}
...
export function setSession(groupFolder: string, sessionId: string): void {
  db.prepare("INSERT OR REPLACE INTO sessions (group_folder, session_id) VALUES (?, ?)").run(
    groupFolder,
    sessionId,
  );
}
